...
export function walkAst(node: AnyNode | undefined, visitor: (node: AnyNode, parent?: AnyNode) => void, parent?: AnyNode): void {
...
export function collectTopLevelFunctions(programNode: AnyNode): AnyNode[] {
...
export function attachParents(node: AnyNode | undefined, parent?: AnyNode): void {
  if (!node || typeof node.type !== 'string') {
    return;
  }
...
export function getFunctionName(node: AnyNode, parent?: AnyNode, fallbackIndex = 1): string {
  if (node.id?.name) {
    return node.id.name;
  }
